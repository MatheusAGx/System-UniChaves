...
if (isset($_POST['filtrar'])) 
{
   ...
   $q_busca_usuario = "SELECT *, (SELECT descricao FROM usuarios_tipo WHERE id = id_usuario_tipo) AS tipo, (SELECT descricao FROM usuarios_instituicao WHERE id = id_instituicao) AS instituicao FROM usuarios ";
    ...
    if (!empty($nome)) {
        $q_busca_usuario .= "WHERE nome LIKE '%$nome%' ORDER BY nome ASC ";
    
    } else if (!empty($cpf)) {
        $q_busca_usuario .= "WHERE cpf LIKE '%$cpf%' ORDER BY nome ASC";

    } else if (!empty($email)) {
        $q_busca_usuario .= "WHERE email LIKE '%$email%' ORDER BY nome ASC";

    } else if (!empty($telefone)) {
        $q_busca_usuario .= "WHERE telefone LIKE '%$telefone%' ORDER BY nome ASC";
